        ...
        <div class="col">
            ...
            <div class="row">
                <div class="col">
                    ...
                    <h3>Search by m/z (mass-to-charge ratio)</h3>
                    <form @submit.prevent="searchByMZ">
                        <div class="mb-3">
                            <label for="mz" class="form-label">m/z value</label>
                            <input x-model="mzValue" type="text" class="form-control" id="mz" name="mz" required
                                   placeholder="756.41">
                        </div>
                        <div class="mb-3">
                            <label for="charge" class="form-label">Charge (z)</label>
                            <select x-model.number="charge" class="form-select" id="charge" name="charge" required>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input x-model="tolerance" type="text" class="form-control" id="tolerance"
                                       name="tolerance" placeholder="0.02" required>
...
<script>
    ...
    function searchData() {
        ...
        return {
            ...
            calculateMassFromMZ() {
                try {
                    if (!this.mzValue || isNaN(this.mzValue) || !this.charge || isNaN(this.charge)) {
                        return {error: "m/z i naboj moraju biti ispravni brojevi"};
                    }
            ...
            // Nova funkcija za pretraživanje prema m/z vrijednosti
            async searchByMZ() {
                this.error = null;
                this.loading = true;

                try {
                    const result = this.calculateMassFromMZ();
                    if (result.error) {
                    ...
                    if (isNaN(toleranceValue) || toleranceValue <= 0) {
                        this.error = "Tolerancija mora biti pozitivan broj";
                        return;
                    }

                    // Izračun raspona mase na temelju tolerancije
                    let massFrom, massTo;

                    if (this.toleranceUnit === "ppm") {
                        // Pretvaranje ppm u apsolutnu vrijednost
                    ...
                    // Postavi parametre i pokreni pretraživanje
                    ...
                    console.error('Error in search by m/z:', error);
