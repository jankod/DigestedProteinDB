...
public class BinaryPeptideDbUtil {
    ...
    //static AccessionDbReader accDb = new AccessionDbReader("../custom_accession.db");
    //  System.out.println("Loading accession DB: count" + accDb.getAccessionCount());
        ...
        try {
            ...
            while (start < value.length()) {
                ...
                for (String acc : accessions) {
                    int accIndex = Integer.parseInt(acc);

                    //String accession = accDb.getAccession(accIndex);
                    //byte[] accBytes = accession.getBytes(StandardCharsets.US_ASCII); // npr. "A0A85SMEL6" = 10 bajtova
                    //buffer.put(accBytes);

                    writeVarInt(buffer, accIndex);
                }
                start = dashIndex + 1;
            }
